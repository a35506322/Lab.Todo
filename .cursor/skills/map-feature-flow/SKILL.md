...
description: 把一個功能/頁面/API endpoint 翻譯成「考前重點」式的檔案地圖：快速導覽段一眼看懂、條列為主、每條掛檔案位置但不解釋程式碼。使用時機：使用者要快速了解某個功能/頁面在做什麼、呼叫哪些 API、做了什麼判斷，例如「登入流程怎麼跑」「XxxPage 在幹嘛」「XxxEndpoint 是被誰用的」。支援任何前後端框架（React / Vue / Svelte / Angular + Express / FastAPI / Spring / .NET / Rails / Gin 等）。
---
...
**展開的章節都一樣**（快速導覽 / 入口 / 操作 / API / 判斷 / 成功後 / 順帶會發生的事）。差別只在「**快速導覽用哪個角度敘述**」與「**入口段是否要列多個呼叫者**」。
...
4. **「（無）」的順帶會發生的事類型直接砍**
    - 沒有 Email 順帶會發生的事 → 整列刪除，不要寫「Email：（無）」
5. **章節按需出現**
    - 沒內容的章節（如「順帶會發生的事」全空）整個砍掉，不留空殼
